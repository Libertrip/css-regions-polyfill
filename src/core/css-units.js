...
module.exports = (function(window, document) {
  ...
  var cssUnits = {
    ...
    convertToPixels: function convertToPixels(cssLength, element, opts) {
      ...
      if (!converter)
        throw new Error("No suitable conversion from unit '" + currentUnit + "' to unit 'px'")

      var convertedLength = converter.call(
        null,
        currentLength,
    ...
    convertFromPixels: function convertFromPixels(pixelLength, destinUnit, element, opts) {
      ...
      if (!converter)
        throw new Error("No suitable conversion to unit '" + destinUnit + "' from unit 'px'")
  ...
  cssUnits.convertToPixels.converters = {
    ...
    '%': function(x, e, opts) {
      ...
      var box = e
        ? cssUtils.getBox(e, opts.boxType)
        : {
            top: 0,
            left: 0,
            width: 0,
